...
class Point3DLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, datapath_prefix='data', voxel_size=0.05,
        ...
        # self.data_paths = sorted(glob(join(datapath_prefix, split, 'RPmz2sHmrrY_*.pth')))
    ...
    def __getitem__(self, index_long):
        ...
        if self.use_shm:
            ...
            labels_in = labels_in.astype(np.int16)
            # no color in the input point cloud, e.g nuscenes
        ...
        locs, feats, labels, _, inds_reconstruct = self.voxelizer.voxelize(
