    ...
    def __getitem__(self, index_long):
        ...
        if self.use_shm:
            ...
            if 'scannet_200' in self.dataset_name:
                ...
                paint_label_path = self.data_paths[index].replace('scannet_200', 'scannet_200_ram_sam_paint').replace('_vh_clean_2.pth','.pth')
            elif 'nuscenes' in self.dataset_name:
                locs_in, feats_in, labels_in = torch.load(self.data_paths[index])
                paint_label_path = self.data_paths[index].replace('nuscenes_3d', 'nuscenes_2d/nuscenes_3d_fcclip_paint')
            elif 'matterport' in self.dataset_name:
                locs_in, feats_in, labels_in = torch.load(self.data_paths[index])
                paint_label_path = self.data_paths[index].replace('matterport_3d', 'matterport_3d_fcclip_paint')
            else:
                locs_in, feats_in, labels_in = torch.load(self.data_paths[index])
                # paint_label_path = self.data_paths[index].replace('nuscenes_3d', 'nuscenes_2d/nuscenes_3d_fcclip_paint')
                paint_label_path = self.data_paths[index].replace('scannet_3d', 'scannet_3d_fcclip_sam_paint_v2').replace('_vh_clean_2.pth','.pth')
            
            if self.split == 'train':
                paint_labels_in = torch.load(paint_label_path)
            else:
                paint_labels_in = 0
        ...
        if self.split == 'train' and flag_mask_merge:
            locs, feats, labels, paint_labels, inds_reconstruct, vox_ind = self.voxelizer.voxelize(
                locs_in, feats_in, labels_in, paint_labels=paint_labels_in, return_ind=True)
            ...
            locs, feats, labels, paint_labels, inds_reconstruct, vox_ind = self.voxelizer.voxelize(
                locs_in, feats_in, labels_in, paint_labels_in, return_ind=True)
            ...
            locs, feats, labels, _, inds_reconstruct, vox_ind = self.voxelizer.voxelize(
        ...
        # paint_labels = torch.from_numpy(paint_labels).long()
        
        if self.eval_all:
            return coords, feats, labels, feat_3d, mask, torch.from_numpy(inds_reconstruct).long()
            # return coords, feats, labels, None, feat_3d, mask, torch.from_numpy(inds_reconstruct).long()
        return coords, feats, labels, paint_labels, feat_3d, mask
...
def collation_fn(batch):
    ...
    '''
    coords, feats, labels, paint_labels, feat_3d, mask_chunk = list(zip(*batch))

    for i in range(len(coords)):
        coords[i][:, 0] *= i

    return torch.cat(coords), torch.cat(feats), torch.cat(labels), torch.cat(paint_labels),\
